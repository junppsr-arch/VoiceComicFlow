...
# [AI Constraint] 1. 画像表示に必要な最低限のライブラリのみ最速インポート
import cv2
import numpy as np
import fitz  # PyMuPDF
import time
...
# [AI Constraint] 2. PDFを開いて1ページ目だけを最速で画面に叩き出す（プレビュー表示）
try:
    doc = fitz.open(PDF_PATH)
    page0 = doc[0]
    zoom0 = 1400 / page0.rect.width
    mat0 = fitz.Matrix(zoom0, zoom0)
    pix0 = page0.get_pixmap(matrix=mat0, alpha=False)
    img0_bgr = cv2.cvtColor(np.frombuffer(pix0.samples, dtype=np.uint8).reshape(pix0.h, pix0.w, 3), cv2.COLOR_RGB2BGR)
    
    sh, sw = img0_bgr.shape[:2]
    startup_display = np.zeros((sh, sw + 320, 3), dtype=np.uint8) # 320はHUD_WIDTHの固定値
    startup_display[:, :sw] = img0_bgr
    startup_display[:, sw:] = (40, 40, 40)
    cv2.putText(startup_display, "Loading System...", (sw + 50, sh//2), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (200,200,200), 2)
    
    cv2.namedWindow("VoiceComicFlow", cv2.WINDOW_NORMAL | cv2.WINDOW_KEEPRATIO)
    
    # モニターに合わせた暫定リサイズ（UI崩れ防止）
    init_h = sh
    init_w = sw + 320
    if init_h > 980:
        s = 980 / init_h
        init_w, init_h = int(init_w * s), int(init_h * s)
    cv2.resizeWindow("VoiceComicFlow", init_w, init_h)
    
    cv2.imshow("VoiceComicFlow", startup_display)
    cv2.waitKey(1)
    print("✅ プレビュー表示完了。残りのシステムをロードします...")
except Exception as e:
    from tkinter import messagebox
    messagebox.showerror("エラー", f"PDFを開けませんでした:\n{e}")
    sys.exit()

# 3. 残りの重いライブラリ（AI、Pillow等）をロード
...
def save_project_palette(palette, path):
    global cached_hud_text_layer
    cached_hud_text_layer = None # パレット更新時に文字キャッシュを破棄
    try:
...
def request_canvas_render():
    global need_canvas_render
    need_canvas_render = True

def request_hud_render():
    global need_hud_render
    need_hud_render = True

def request_render():
    # 互換性・全体更新用
    global need_canvas_render, need_hud_render
    need_canvas_render = True
    need_hud_render = True
...
def trigger_page_change(new_idx):
    global page_idx, last_scroll_time
...
# 3. 初期化とデータ読み込み (PDF, YOLO, 設定)
# ─────────────────────────────────────────
# ※PDFとdocのオープンは、最速プレビューのためスクリプト先頭で実行済み
...
def prefetch_pages(current_idx):
    ...
    # 定数に基づき、前後 PREFETCH_PAGES ページをメモリに維持する
    keep_range = set(range(max(0, current_idx - PREFETCH_PAGES), min(len(doc), current_idx + PREFETCH_PAGES + 1)))
...
current_page_pos = None   # 現在のページのページ番号カスタム座標
gui_busy       = False    # サブウィンドウ展開中の入力ブロック用フラグ
last_scroll_time = 0.0    # 高速スクロール判定用のタイムスタンプ
...
need_canvas_render = True
need_hud_render = True
cached_hud_text_layer = None # 文字部分のみの透明画像キャッシュ
...
def load_page(idx: int):
    global img, h, w_orig, bboxes, yolo_running, drawn_actions, number_counter, next_group_id, current_page_pos
    ...
    mem = GLOBAL_MEMORY.get(idx, {"drawn_actions": [], "number_counter": 1, "page_pos": None})
    drawn_actions = copy.deepcopy(mem.get("drawn_actions", []))
    number_counter = mem.get("number_counter", 1)
    current_page_pos = mem.get("page_pos", None)
    PAGE_IS_DIRTY = False
    ...
    with yolo_lock:
        bboxes = []
        yolo_running = yolo_enabled and (model is not None)
    ...
    if not yolo_running:
        ...
        def _run_yolo(frame, p_idx):
            ...
            time.sleep(GEAR3_YOLO_DELAY)  # 3速: ユーザーが作業を開始するまでYOLO起動を意図的に大きく遅延
            
            # [AI Constraint] 高速ページ遷移によるゾンビスレッド対策
            if not yolo_enabled or model is None or page_idx != p_idx:
                with yolo_lock:
                    if page_idx == p_idx:
                        bboxes = []
                        yolo_running = False
...
class TextShape(Shape):
    ...
    def check_hover(self, mx, my):
        if len(self.region) < 4: return None, None
        x1, y1, x2, y2 = self.region[0], self.region[1], self.region[2], self.region[3]
        if x1 <= mx <= x2 + TEXT_PAD_X*2 and y1 <= my <= y2 + TEXT_PAD_Y*2:
            return self, 'center'
        return None, None
...
def get_page_number_bbox():
    ...
    if 1 <= logical_num <= TOTAL_PAGES:
        ...
        d = ImageDraw.Draw(dummy_img)
        try:
            bbox = d.textbbox((0, 0), page_text, font=PAGE_FONT)
            tw, th = bbox[2] - bbox[0], bbox[3] - bbox[1]
        except:
            tw, th = 80, 40
        
        if 'current_page_pos' in globals() and current_page_pos:
            px, py = current_page_pos
        else:
            px = max(10, w_orig - tw - PAGE_POS_MARGIN_X)
            py = PAGE_POS_MARGIN_Y
        return [px, py, px + tw, py + th, page_text]
    return None
...
def render_texts_to_layer(layer):
    ...
    p_info = get_page_number_bbox()
    if p_info:
        px, py, _, _, page_text = p_info
...
def get_deletable_action(mx, my, margin=15):
    for action in reversed(drawn_actions):
        ...
        if t in ["box", "manual_text"] and len(region) >= 4:
            x1, y1, x2, y2 = region[0], region[1], region[2], region[3]
            if min(x1, x2) <= mx <= max(x1, x2) and min(y1, y2) <= my <= max(y1, y2):
                return action
        elif t == "text" and len(region) >= 4:
            x1, y1, x2, y2 = region[0], region[1], region[2] + TEXT_PAD_X*2, region[3] + TEXT_PAD_Y*2
...
def render():
    global cached_display, need_canvas_render, need_hud_render, last_scroll_time
    global curr_scale, pad_x, pad_y
    if img is None: return

    # カスタム設定値で3段変速ロードを制御
    time_since_scroll = time.time() - last_scroll_time if 'last_scroll_time' in globals() else 999.0
    is_gear1 = time_since_scroll < GEAR2_DELAY

    if cached_display is None or cached_display.shape[:2] != (h, w_orig + HUD_WIDTH):
        cached_display = np.zeros((h, w_orig + HUD_WIDTH, 3), dtype=np.uint8)
        need_canvas_render = True
        need_hud_render = True

    if is_gear1:
        preview_display = cached_display.copy()
        preview_display[:, :w_orig] = img
        
        # [AI Constraint] ダーティ・レクタングル手法：HUDのページ番号エリアだけを部分的に黒塗りして即時更新（負荷ゼロ）
        hud_x = w_orig + 10
        y_pos = HUD_TEXT_MARGIN_Y
        logical_num = page_idx - LOGICAL_PAGE_OFFSET + 1
        page_text = f"Page: {logical_num:02d} / {TOTAL_PAGES:02d}" if 1 <= logical_num <= TOTAL_PAGES else (f"Cover (P{page_idx+1})" if logical_num < 1 else f"Extra (P{logical_num:02d})")
        cv2.rectangle(preview_display, (w_orig, y_pos-25), (w_orig + HUD_WIDTH, y_pos + 5), (40, 40, 40), -1)
        put_hud_text(preview_display, f"▼ {page_text}", hud_x, y_pos, fg=(0, 255, 255))

        # [AI Constraint] 超軽量の1速描画（アルファ合成・Pillow文字・HUD全体更新をスキップし、枠と線だけを直接描く）
        for act in drawn_actions:
            ...
            if stype == "box" and len(act.region) >= 4:
                ...
                if getattr(act, "shape", "rect") == "ellipse":
                    ...
                    axes = (abs(x2 - x1) // 2, abs(y2 - y1) // 2)
                    if axes[0] > 0 and axes[1] > 0:
                        cv2.ellipse(preview_display, (cx, cy), axes, 0, 0, 360, c, BOX_THICKNESS)
                else:
    ...
    if need_hud_render:
        display = cached_display # HUD描画ロジックをそのまま活かすためのエイリアス
        hud_x = w_orig + 10
        ...
        # --- 1. OpenCVによる動的背景・スウォッチの高速描画 ---
        temp_y = y
        for i in range(len(PALETTE)):
            item_y_range = (temp_y - 25, temp_y + 5)
            ...
            if display_inverted:
                ...
                cv2.rectangle(display, (hud_x, temp_y-20), (hud_x+30, temp_y), swatch_color, 2)
            else:
                cv2.rectangle(display, (hud_x, temp_y-20), (hud_x+30, temp_y), swatch_color, -1)
            
            temp_y += HUD_TEXT_LINE_SPACING + 5
            if temp_y > h - 50: break

        # --- 2. Pillowによるテキストレイヤーのキャッシュ生成（必要な時だけ） ---
        if cached_hud_text_layer is None or cached_hud_text_layer.shape[:2] != (h, HUD_WIDTH):
            hud_text_img = Image.new("RGBA", (HUD_WIDTH, h), (0, 0, 0, 0))
            d = ImageDraw.Draw(hud_text_img)

            d.text((10, char_header_y - 18), "キャラクター", font=JP_FONT, fill=(200, 200, 200, 255))
            
            trash_fg = (0, 0, 255, 255) if char_delete_mode else (150, 150, 150, 255)
            trash_text = "[x]削除" if not char_delete_mode else "[DELETE]"
            d.text((HUD_WIDTH - 80, char_header_y - 18), trash_text, font=JP_FONT, fill=trash_fg)
            ...
            for i in range(len(PALETTE)):
                ...
                elif display_inverted: text_fill = (swatch_color[2], swatch_color[1], swatch_color[0], 255)
                
                d.text((40, text_y - 18), p_item["name"], font=JP_FONT, fill=text_fill, stroke_width=2, stroke_fill=(0,0,0,255))
                
                text_y += HUD_TEXT_LINE_SPACING + 5
                if text_y > h - 50: break

            cached_hud_text_layer = cv2.cvtColor(np.array(hud_text_img), cv2.COLOR_RGBA2BGRA)

        # --- 3. キャッシュ画像のアルファ合成（毎フレーム超高速） ---
        alpha_t = cached_hud_text_layer[:, :, 3:4].astype(np.float32) / 255.0
        hud_rgb = cached_hud_text_layer[:, :, :3].astype(np.float32)
        ...
        need_hud_render = False

    display_out = cached_display.copy()

    if mouseX < w_orig:
        if dragging:
            if resizing_action == "page_pos":
                p_info = get_page_number_bbox()
                if p_info:
                    px1, py1, px2, py2, _ = p_info
                    cv2.rectangle(display_out, (int(px1)-5, int(py1)-5), (int(px2)+5, int(py2)+5), (255, 255, 255), 2)
            ...
            if ha and not alt_pressed:
                ...
                if s_type == "eraser" and len(region) >= 2:
                    ...
                    elif s_type == "text" and len(region) >= 4:
                        tx1, ty1, tx2, ty2 = region[0], region[1], region[2], region[3]
                        cv2.rectangle(display_out, (int(tx1), int(ty1)), (int(tx2 + TEXT_PAD_X*2), int(ty2 + TEXT_PAD_Y*2)), (0, 255, 255), 2)
                    elif s_type == "manual_text" and len(region) >= 4:
                        tx1, ty1, tx2, ty2 = region[0], region[1], region[2], region[3]
                        cv2.rectangle(display_out, (int(tx1), int(ty1)), (int(tx2), int(ty2)), (0, 255, 255), 2)
            else:
                if mode == "delete":
                    ...
                    if p_info:
                        px1, py1, px2, py2, _ = p_info
                        if px1 - 10 <= mouseX <= px2 + 10 and py1 - 10 <= mouseY <= py2 + 10:
                            cv2.rectangle(display_out, (int(px1)-5, int(py1)-5), (int(px2)+5, int(py2)+5), (0, 255, 255), 2)
...
def open_export_dialog():
    ...
    tree = ttk.Treeview(tree_frame, columns=("check", "name", "count", "pages"), show="headings", height=8, yscrollcommand=scroll_y.set, xscrollcommand=scroll_x.set)
...
def handle_hud_click(mx, my, flags):
    ...
    if 'current_palette_regions' in globals():
        if 'trash_icon_y_range' in globals() and trash_icon_y_range[0] <= my <= trash_icon_y_range[1]:
            ...
            cached_hud_text_layer = None
            request_hud_render(); return
...
def mouse_callback(event, x, y, flags, param):
    # [AI Constraint] 関数の途中で宣言すると SyntaxError になるため、必ず先頭で一括宣言すること。
    global mouseX, mouseY, dragging, drag_start, drag_end, number_counter, redo_stack, page_idx
    global resizing_action, resizing_part, drag_start_region, shift_pressed, current_page_pos, gui_busy
    if gui_busy: return
    ...
    if event == cv2.EVENT_LBUTTONDOWN and ix < w_orig:
        ...
        if mode == "numbering":
            ...
            if p_info:
                px1, py1, px2, py2, _ = p_info
                if px1 - 10 <= ix <= px2 + 10 and py1 - 10 <= iy <= py2 + 10:
                    dragging = True
                    drag_start = (ix, iy)
        ...
        if dragging and ix < w_orig:
            ...
            if resizing_action == "page_pos":
                dx = ix - drag_start[0]
                dy = iy - drag_start[1]
                current_page_pos = (drag_start_region[0] + dx, drag_start_region[1] + dy)
                mark_dirty()
                render()
            elif mode == "eraser" and not resizing_action:
                render()
...
def _async_save_bg(p_idx, actions, n_cnt, p_pos=None):
...
def save_current_page():
    ...
    global PAGE_IS_DIRTY, current_page_pos
    ...
    GLOBAL_MEMORY[page_idx] = {
        ...
        "page_pos": current_page_pos,
        "is_dirty": False
    }
    threading.Thread(target=_async_save_bg, args=(page_idx, drawn_actions, number_counter, current_page_pos), daemon=True).start()
    PAGE_IS_DIRTY = False
...
def get_character_stats():
    ...
    # [AI Constraint] 物理ページ全体を走査しないと、オフセット設定時に集計漏れが発生するため range(len(doc)) に修正
    for p in range(len(doc)):
        logical_num = p - LOGICAL_PAGE_OFFSET + 1
...
def export_to_pdf(target_pages=None):
    ...
    global h, w_orig, drawn_actions, page_idx, current_page_pos
    ...
    _saved_page_pos = current_page_pos
        ...
        if os.path.exists(state_file):
            try:
                with open(state_file, "r", encoding="utf-8") as f:
                    ...
                    GLOBAL_MEMORY[p] = {"drawn_actions": [a for a in actions if a], "number_counter": data.get("number_counter", 1), "page_pos": data.get("page_pos", None), "is_dirty": False}
            except: pass
        if p not in GLOBAL_MEMORY:
            GLOBAL_MEMORY[p] = {"drawn_actions": [], "number_counter": 1, "page_pos": None, "is_dirty": False}
...
while True:
    ...
    if pending_gui_action:
        ...
        param = pending_gui_param
        pending_gui_action = None
        pending_gui_param = None
        
        try:
            if action == "color":
                run_color_chooser(param)
            elif action == "add":
                run_add_char_dialog()
            ...
            dragging = False
            resizing_action = None
            # [AI Constraint] ダイアログ展開中にOS側で溜まっていたマウス・キー入力をすべて破棄(フラッシュ)する。
            # これがないと、ダイアログを閉じた瞬間に溜まった操作が暴発する。
    ...
    if key in (KEY_PAGE_NEXT, 13):
        ...
        pending_gui_action = "load_palette"
        request_render()
